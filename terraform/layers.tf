...
resource "aws_s3_object" "apiLayerZipObject" {
  bucket = aws_s3_bucket.tobiLambdaCode.id
...
resource "aws_lambda_layer_version" "tobiLambdaLayer" {
  layer_name          = "tobiLambdaLayer-${terraform.workspace}"
  s3_bucket           = aws_s3_bucket.tobiLambdaCode.bucket
